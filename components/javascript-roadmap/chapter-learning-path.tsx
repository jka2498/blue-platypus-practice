...
export function ChapterLearningPath({
  ...
  const allMcqsDone = totalQuestions === 0 || (questionIndex >= totalQuestions && !currentQuestion);
  ...
  return (
    <div className="space-y-6">
      ...
      <ol className="grid gap-2 rounded-xl border bg-surface p-3 text-sm shadow-card sm:grid-cols-3">
        ...
        >
          <span className="inline-flex items-center gap-1.5 font-semibold">
            {stage === "summary" ? <PlayCircle className="h-4 w-4" /> : <CheckCircle2 className="h-4 w-4" />}
            Lesson
          </span>
        </li>
        <li
          className={cn(
            ...
            stage === "challenge" && "border-success/40 bg-success/10 text-success",
            stage === "summary" && "border-border text-muted-foreground",
          )}
        >
          <span className="inline-flex items-center gap-1.5 font-semibold">
            {stage === "summary" ? <Lock className="h-4 w-4" /> : stage === "mcq" ? <PlayCircle className="h-4 w-4" /> : <CheckCircle2 className="h-4 w-4" />}
            MCQ Checkpoint
          </span>
        </li>
        <li
        ...
        >
          <span className="inline-flex items-center gap-1.5 font-semibold">
            {stage === "challenge" ? <PlayCircle className="h-4 w-4" /> : <Lock className="h-4 w-4" />}
            Coding Challenge
          </span>
        </li>
      </ol>
      ...
      {stage === "summary" ? (
        <section className="space-y-4" aria-labelledby="chapter-summary-heading">
          ...
          <Button type="button" onClick={() => setStage("mcq")}>Start MCQs</Button>
        </section>
      ) : null}

      {stage === "mcq" ? (
        <section className="space-y-4" aria-labelledby="chapter-mcqs-heading">
          <h2 id="chapter-mcqs-heading" className="heading-3 inline-flex items-center gap-2">
          ...
          {totalQuestions === 0 ? (
            <div className="space-y-4 rounded-xl border bg-surface p-5 shadow-card">
              ...
              <Button type="button" onClick={() => setStage("challenge")}>Continue to Coding Challenge</Button>
            </div>
          ) : allMcqsDone ? (
            <div className="space-y-4 rounded-xl border bg-surface p-5 shadow-card">
              ...
              <p className="text-sm text-muted-foreground">
                You scored {correctCount} / {totalQuestions}. Ready for a topic-specific coding challenge?
              </p>
              <Button type="button" onClick={() => setStage("challenge")}>Continue to Coding Challenge</Button>
            </div>
          ) : currentQuestion ? (
            <div className="space-y-4 rounded-xl border bg-surface p-5 shadow-card">
              ...
              <div className="flex flex-wrap gap-2">
                ...
                <Button type="button" variant="secondary" onClick={handleNextQuestion} disabled={!revealed}>
          ...
          {challenge ? (
            <article className="space-y-4 rounded-xl border bg-surface p-5 shadow-card">
              <div className="flex flex-wrap items-center gap-2">
                ...
                <Badge variant={difficultyVariant}>{challenge.difficulty}</Badge>
              ...
              <div className="text-sm text-muted-foreground">
                {challenge.source === "database" ? <Markdown>{challenge.prompt}</Markdown> : <p>{challenge.prompt}</p>}
              </div>
