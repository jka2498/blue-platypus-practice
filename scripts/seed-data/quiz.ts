...
const EASY_QUIZ_ADDITIONS: SeedQuizQuestion[] = [
  makeQuiz(
    ...
    "What does `typeof notDeclared` return?",
  ...
  makeQuiz(
    ...
    "What happens when you reassign a `const` variable?",
    ["It works if the value type matches", "It throws a TypeError", "It silently fails", "It creates a new variable"],
    1,
    "`const` bindings cannot be reassigned. Reassignment throws.",
  ),
  makeQuiz(
    "array-methods",
    "q-easy-extra-03-filter",
    "`[1,2,3,4].filter(n => n % 2 === 0)` returns:",
    ["[1,3]", "[2,4]", "2", "true"],
    1,
  ...
  makeQuiz(
    ...
    "Which destructuring swaps `a` and `b`?",
  ...
  makeQuiz(
    ...
    ["`import sum from './math'`", "`import { sum } from './math'`", "`import * as sum from './math'`", "`require('./math').sum`"],
    1,
